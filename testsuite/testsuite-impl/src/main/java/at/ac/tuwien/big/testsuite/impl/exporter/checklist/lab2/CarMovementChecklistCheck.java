 ...
 */
public class CarMovementChecklistCheck extends SeleniumChecklistCheck {
    ...
    public void passValidationResultEntry(ValidationResultEntry validationResultEntry) {
        if (inSeleniumResultType) {
            ...
            if (title != null) {
                if ((title.startsWith(TestsuiteConstants.KNOWN_ERROR_PREFIX + " Incorrect Player position") || validationResultEntry.getTitle().startsWith("Incorrect Computer position")) || !title.startsWith(TestsuiteConstants.KNOWN_ERROR_PREFIX)) {
                    if (points > 0) {
                        points = 0;
                    }
                }
