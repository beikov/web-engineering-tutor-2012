...
/**
 * Checks for occurance of oily move test errors
 *
 * @author Florian Zoubek
 */
public class OilyMoveChecklistCheck extends SeleniumChecklistCheck {
    ...
    public void passValidationResultEntry(ValidationResultEntry validationResultEntry) {
        if (inSeleniumResultType) {
            ...
            if (title != null) {
                if (title.startsWith(TestsuiteConstants.KNOWN_ERROR_PREFIX + " Oily field never reached") || !title.startsWith(TestsuiteConstants.KNOWN_ERROR_PREFIX)) {
                    if (points > 0) {
                        points = 0;
                    }
                }
    ...
    public String getDescription() {
        return "Oily field reached during tests";
    }
}
