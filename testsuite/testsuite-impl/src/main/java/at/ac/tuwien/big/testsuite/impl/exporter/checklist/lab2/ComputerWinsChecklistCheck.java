...
/**
 * Checks for occurance of "computer could not win" test errors
 *
 * @author Florian Zoubek
 */
public class ComputerWinsChecklistCheck extends SeleniumChecklistCheck {
    ...
    public void passValidationResultEntry(ValidationResultEntry validationResultEntry) {
        if (inSeleniumResultType) {
            ...
            if (title != null) {
                if (title.startsWith(TestsuiteConstants.KNOWN_ERROR_PREFIX + " Computer couldn't win") || !title.startsWith(TestsuiteConstants.KNOWN_ERROR_PREFIX)) {
                    if (points > 0) {
                        points = 0;
                    }
                }
    ...
    public String getDescription() {
        return "Computer has won during tests.";
    }
}
