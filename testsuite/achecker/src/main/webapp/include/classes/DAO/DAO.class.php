...
class DAO {
	...
	// private
	static private $db = @mysql_connect(DB_HOST . ':' . DB_PORT, DB_USER, DB_PASSWORD);     // global database connection
	
	function DAO()
	{
		if (!isset($this->db))
		{
                        // Workaround for stupid programmers error
			$this->db = self::$db;//@mysql_connect(DB_HOST . ':' . DB_PORT, DB_USER, DB_PASSWORD);
			if (!$this->db) {
